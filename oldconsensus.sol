...
contract Consensus is ReentrancyGuard, Ownable {
    ...
    IERC20 public dxpToken = IERC20(0x08Eb4d9c6e388777b21b517A13030a08e39AC279); // Hardcoded DXP token address
    ...
    // Reward tracking
    uint256 public rewardRate = 11574074074074; // 1 token per day (1e18 / 86400)
    ...
    constructor() Ownable(msg.sender) {
        ...
        for (uint256 i = 1; i <= 8; i++) {
            ...
            farmScores[i] = 1000;     // Initial score also set to 10%
    ...
    function activateRound(uint256 roundNumber) external {
    ...
    function completeRound(uint256 roundNumber) external {
        ...
        // Award leader bonus (small incentive for completing a round)
        VerifierMetrics storage metrics = verifierMetrics[roundLeader[roundNumber]];
        metrics.accumulatedRewards += 1e16; // 0.01 tokens as bonus
     ...
     */
    function increaseVerifierStake(address verifierAddress, uint256 additionalStake) external nonReentrant {
     ...
     */
    function withdrawVerifierStake(address verifierAddress, uint256 amount) external nonReentrant {
    ...
    function submitFarmScore(uint256 farmId, uint256 score, uint256 roundNumber)
        ...
        validFarmId(farmId)
    {
        require(score <= 10000, "Score cannot exceed 100%");
        ...
        uint256 newRewards = timeSinceLastActive * rewardRate;
        metrics.accumulatedRewards += newRewards;
     ...
     */
    function claimRewards() external onlyRegisteredVerifier nonReentrant {
        VerifierMetrics storage metrics = verifierMetrics[msg.sender];
        
        // Calculate final rewards including time since last activity
        uint256 timeSinceLastActive = block.timestamp - metrics.lastActiveTimestamp;
        uint256 uptimeRewards = timeSinceLastActive * rewardRate;
        uint256 totalRewards = metrics.accumulatedRewards + uptimeRewards;
        
        require(totalRewards > 0, "No rewards to claim");
        
        // Reset accumulated rewards
        metrics.accumulatedRewards = 0;
        metrics.lastActiveTimestamp = block.timestamp;
        metrics.lastRewardsClaim = block.timestamp;
        
        // Update claimed rewards
        claimedRewards[msg.sender] += totalRewards;
        
        // Transfer rewards
        bool success = dxpToken.transfer(msg.sender, totalRewards);
     ...
     * @return lastRewardsClaim Last rewards claim timestamp
     */
    function getVerifierMetrics(address verifier) external view returns (
        ...
        return (
            ...
            metrics.totalUptime,
            metrics.accumulatedRewards,
            metrics.lastRewardsClaim
        );
    }
    ...
    /**
     * @notice Calculate pending rewards for a verifier
     * @param verifier The address of the verifier
     * @return pendingRewards Pending rewards amount
     */
    function calculatePendingRewards(address verifier) external view returns (uint256 pendingRewards) {
        if (!registeredVerifiers[verifier]) {
            return 0;
        }
        
        VerifierMetrics storage metrics = verifierMetrics[verifier];
        
        // Calculate rewards from uptime since last activity
        uint256 timeSinceLastActive = block.timestamp - metrics.lastActiveTimestamp;
        uint256 uptimeRewards = timeSinceLastActive * rewardRate;
        
        return metrics.accumulatedRewards + uptimeRewards;
    }
     ...
     */
    function checkAndUpdateVerifierStatus(address verifierAddress) external returns (bool) {
     ...
     * @param newRate New reward rate (tokens per day in wei)
     */
